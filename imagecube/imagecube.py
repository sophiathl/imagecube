...
def merge_headers(montage_hfile, orig_header):
    '''
    Merges an original image header with the WCS info
    in a header file generated by montage.mHdr.
    Replaces the original montage output with the merged file.
...
def register_images(images_with_headers):
    ...
    for i in range(0, len(images_with_headers)):
        ...
        merge_headers(artificial_filename, images_with_headers[i][1])
